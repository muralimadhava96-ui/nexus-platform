...
// ═══════════════════════════════════════════════════════════════════════════
//  AI SERVICE  — calls Anthropic API via Vercel proxy
// ═══════════════════════════════════════════════════════════════════════════
async function callAI(messages, systemPrompt) {
  ...
  try {
    ...
    throw new Error(`Network error — check your connection and try again. ${err.message}`);
  }
  if (!resp.ok) {
    const errData = await resp.json().catch(() => ({}));
    throw new Error(`API error ${resp.status}: ${errData?.error || resp.statusText}`);
  }
...
function AppShell() {
  ...
  // Apply theme tokens synchronously before children render
